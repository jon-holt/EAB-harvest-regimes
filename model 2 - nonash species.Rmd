...
```{r, echo=FALSE}
# subset the data
model_df <- subset(df_m, select = c(harvest_nonash, spAGB_MgHa_prev_nonash, percBAremv_nonash, EAB, inv_period, pop_den, MHHI))
...
X <- model.matrix(percBAremv_nonash ~ spAGB_MgHa_prev_nonash + EAB + pop_den + MHHI + inv_period, model_df)

# intercept
X[,1] <- rep(1, dim(X)[1])

# response variable
y <- model_df$percBAremv_nonash

# n
n <- length(y)

# indicator variable (binary)
z <- model_df$harvest_nonash 
...
grow_nonash <- jags(data=cutData, param=parNames, n.chains = 3,
...
```{r}
params <- MCMCsummary(grow_nonash,
