# Model 3: Ash fraction of harvest
...
load("data/df_matched.RData.RData")
```
...
```{r, echo=FALSE}
# subset the data
model_df <- subset(df_m, select = c(harvest_ash, fracAGB_ash, fracBAremv_ash, EAB, inv_period, pop_den, MHHI))

# Transform fraction to percent for better visualization later on
model_df$fracAGB_ash <- model_df$fracAGB_ash*100 
...
X <- model.matrix(fracBAremv_ash ~ fracAGB_ash + EAB + pop_den + MHHI + inv_period, model_df)

# intercept
X[,1] <- rep(1, dim(X)[1])

# response variable
y <- model_df$fracBAremv_ash
...
grow_frac <- jags(data=cutData, param=parNames, n.chains = 3,
...
params$upper_mod_90 <- params$mean+1.645*params$sd_mod
params$lower_mod_90 <- params$mean-1.645*params$sd_mod

predictions <- data.frame(intercept = 1,
                       fracAGB_ash = rep(seq(0, 100, length.out=100),2),
...
predictions$EAB_10CI <- params$lower_mod_90[3]
predictions$EAB_90CI <- params$upper_mod_90[3]
...
for (i in 1:dim(predictions)[1]) {
  ...
  predictions$y_95CI[i] <- 1/(1+exp(-predictions$zeta_95CI[i]))
    ...
    #10%
    predictions$zeta_10CI[i] <- predictions$intercept[i] +
    predictions$ABG_mean[i]*predictions$fracAGB_ash[i] +
    predictions$EAB_10CI[i]*predictions$EAB[i] +
    predictions$pop_den_mean[i]*predictions$pop_den[i] +
    predictions$MHHI_mean[i]*predictions$MHHI[i] 
  
  predictions$y_10CI[i] <- 1/(1+exp(-predictions$zeta_10CI[i]))
  
  #90%
    predictions$zeta_90CI[i] <- predictions$intercept[i] +
    predictions$ABG_mean[i]*predictions$fracAGB_ash[i] +
    predictions$EAB_90CI[i]*predictions$EAB[i] +
    predictions$pop_den_mean[i]*predictions$pop_den[i] +
    predictions$MHHI_mean[i]*predictions$MHHI[i]
  
  predictions$y_90CI[i] <- 1/(1+exp(-predictions$zeta_90CI[i]))
}
...
ggplot(df_plot, aes(x=fracAGB_ash, y=y, group=EAB)) + geom_line(aes(color=EAB)) + 
  ...
  ylab("Annual probability of harvesting ash") +
  xlab("fraction aboveground biomass ash") +
  theme_minimal() + 
...
for (i in 1:dim(predictions)[1]) {
  ...
  predictions$y_95CI[i] <- qbeta(0.5, shape1 = predictions$r[i]*predictions$s[i], 
    ...
    predictions$MHHI_mean[i]*predictions$MHHI[i] 
      ...
      predictions$s[i] <- 1/(1+exp(-predictions$zeta_10CI[i]))
  predictions$y_10CI[i] <- qbeta(0.5, shape1 = predictions$r[i]*predictions$s[i], 
                            shape2 = predictions$r[i]*(1-predictions$s[i]))*100
  ...
  #90%
    ...
    predictions$MHHI_mean[i]*predictions$MHHI[i]
      ...
      predictions$s[i] <- 1/(1+exp(-predictions$zeta_90CI[i]))
  predictions$y_90CI[i] <- qbeta(0.5, shape1 = predictions$r[i]*predictions$s[i], 
                            shape2 = predictions$r[i]*(1-predictions$s[i]))*100
}
